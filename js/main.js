...
function buildCVHTML(data, aiContent) {
  ...
  const expHTML = (data.exps||[]).map(e => `
    <div style="margin-bottom:20px;padding-bottom:20px;border-bottom:1px solid #eee">
      ...
      <div style="font-size:.83rem;color:#555;line-height:1.65">${e.aiDesc || e.desc || ''}</div>
    </div>
  `).join('');
  ...
  if (tpl === 'classique') {
    ...
    const expClsHTML = (data.exps||[]).map(e => `
      <div style="margin-bottom:20px">
        ...
        <div style="font-size:.82rem;color:#444;line-height:1.7">${e.aiDesc || (e.desc ? `<ul style="padding-left:16px;margin:0">${e.desc.split('\n').filter(l=>l.trim()).map(l=>`<li style="margin-bottom:3px">${l}</li>`).join('')}</ul>` : '')}</div>
      </div>`).join('');
